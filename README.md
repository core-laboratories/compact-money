...
Compact monetary amounts into structured `{ amount, unit, currency? }` data.
Dependency-free at runtime, with TypeScript types and ESM/CommonJS builds.
The companion [Dart/Flutter package](https://github.com/core-laboratories/compact_money)
uses the same numeric policy and shared test examples.

## Installation

```sh
npm install compact-money
```

Requires Node.js 22 or newer for supported Node usage. The runtime is also
browser-compatible and has no Node-specific imports. Bundlers can tree-shake it.

## Usage

```ts
import { compact } from 'compact-money';

compact({ amount: 80_000, currency: 'VND' });
// { amount: 80, unit: 'k', currency: 'VND' }

const { amount, unit, currency } = compact({ amount: 30_487, currency: 'VND' });
// amount = 30487; unit = ''; currency = 'VND'

compact({ amount: 8_123_456, currency: 'VND' });
// { amount: 8123456, unit: '', currency: 'VND' }

compact({ amount: 8_123_000, currency: 'VND' });
// { amount: 8123, unit: 'k', currency: 'VND' }

compact({ amount: 8_123_000, currency: 'VND', decimals: 3 });
// { amount: 8.123, unit: 'M', currency: 'VND' }

compact({ amount: 8_123_456, allowRounding: true, decimals: 1 });
// { amount: 8.1, unit: 'M' }

compact({ amount: 30_487, currency: 'VND', allowRounding: true });
// { amount: 30.5, unit: 'k', currency: 'VND' }
```

CommonJS: `const { compact } = require('compact-money');`
...
The exported types and primary signature are:

```ts
type CompactUnit = '' | 'k' | 'M' | 'B' | 'T';
interface CompactOptions {
  readonly amount: number;
  readonly currency?: string;
  readonly allowRounding?: boolean; // default: false
  readonly decimals?: number; // integer 0–100; default 0 without rounding
}
...
`compactAmount` is the exact same function as `compact`. Inputs are never mutated.
Omitted currency (including an explicit `undefined` from JavaScript) produces no
`currency` property. Invalid options, non-number or non-finite amounts, invalid
currency, non-boolean rounding flags and invalid decimal limits throw `TypeError`. Strings are never
coerced to numbers. NaN and both infinities are rejected.
...
JavaScript integers beyond `Number.MAX_SAFE_INTEGER` (2⁵³ − 1) can already have
lost precision before this library receives them. BigInt is not accepted.
...
Tests use Node’s built-in `node:test` runner and `node:assert/strict`. TypeScript
tests compile into the ignored `.test-dist/` directory before execution.
`test:coverage` uses Node’s built-in V8 coverage with 100% line, branch and function
thresholds for the compiled library source. No separate test framework is required.

Individual scripts include `build`, `test`, `test:coverage`, `lint`, `format`,
`format:check` and `typecheck`. Package smoke tests install the tarball and check
ESM, CommonJS and both TypeScript declaration entry points.
See [CONTRIBUTING.md](https://github.com/core-laboratories/compact-money/blob/main/CONTRIBUTING.md) for release setup and fixture maintenance.

## License

[CORE License](LICENSE), matching the CORE libraries. npm metadata uses
`SEE LICENSE IN LICENSE` for this custom license.
